...
class CheckmarkWidgetView : HabitWidgetView {
    ...
    fun refresh() {
        ...
        val white = android.graphics.Color.argb(175, 255, 255, 255);
        val blue = android.graphics.Color.argb(255, 0, 255, 0);

        val highContrast = res.getColor(R.attr.contrast100);
        val lowContrast = R.attr.cardBackgroundColor;
//        setShadowAlpha(0x00);
        ...
        when (entryState) {
            ...
            YES_AUTO, NO, UNKNOWN -> {
                ...
                if (freeDays <= 0)
                    ...
                    ring.setText(freeDays.toString());
                label.setTextColor(highContrast);
            }
            else -> {
                ring.setColor(white);
        ...
        ring.setBackgroundColor(transparent)
        // ring.setText(text)
        label.text = name
